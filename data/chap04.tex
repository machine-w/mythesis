...
本系统的核心架构就是一个RESTful的web service。通过这个标准的api，任何系统都可以调用本系统中用户数据和文档数据。同时它也是所有本地客户端程序访问系统的标准接口。关于RESTful api是什么，有什么样的好处，有多么流行，在前文中已经介绍过了。下面就用一个表格~\ref{tab:api}描述一下本系统实现的RESTful api中有那些资源，这些资源获取和操作的方式是什么样的。
...
对于以上URL系统支持多种格式的返回内容，可以返回atom,xml,json等格式，由于篇幅原因这里就不一一展示各种格式的返回内容了。
